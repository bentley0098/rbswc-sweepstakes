-- World Cup 2026 — Reset Teams to ESPN Rankings
--
-- Run this in the Supabase SQL editor to re-seed teams based on ESPN's
-- "All 48 national teams ranked" article (ranks 1–48 → seeds 1–4 in groups of 12).
-- Also clears any existing draw so a fresh draw can be run.

-- ─── CLEAR DRAW STATE ─────────────────────────────────────────────────────────
DELETE FROM draws;
UPDATE app_state SET draw_completed = false, updated_at = NOW() WHERE id = 1;
